...
Actions.prototype.init = function() {
	...
	this.addAction('runModel', function() {	
		webSocket = new WebSocket("ws://localhost:8080/debug");
		
		// d=' + encodeURIComponent(graph.id), 'xml=' + encodeURIComponent(xml)
		
		mxUtils.post('/run', function() {
			mxUtils.alert('error');
		});
		
				
		webSocket.onmessage = function(event) {
			mxUtils.alert(event.data);
		};
		
		
		//mxUtils.alert('sending');
		//webSocket.send("??!!!???!!");	
	});
