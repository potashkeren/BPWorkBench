...
bp.registerBThread("Increaser", function(){
  ...
  while(true){
    ...
    var next = bp.Event(counterEvt.name, {type:counterEvt.data.type, value:counterEvt.data.value +1});
...
bp.registerBThread("capper", function(){
  var countersAt10 = bp.EventSet("done", function(evt){
    return (evt.data == null) ? false : evt.data.type == "counter" && evt.data.value == 10;
  });
